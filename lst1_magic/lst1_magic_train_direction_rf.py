...
parser.add_argument('--output-file', '-o', dest='output_file', type=str, default='./direction_rf.joblib', 
...
direction_estimator.fit(mc_train)
direction_estimator.save(args.output_file)

for reco_param in config['direction_rf']['features'].keys():

    print(f'\nParameter importances ({reco_param}):')
    ...
    for tel_name in config['tel_ids']:
        ...
        for i_par, param in enumerate(config['direction_rf']['features'][reco_param]):
            print(f'    {param}: {direction_estimator.telescope_rfs[reco_param][tel_id].feature_importances_[i_par]}')
