...
PARSER = argparse.ArgumentParser(
    description=("This tools fits the direction random forest regressor on "
...
def compute_separation_angle(shower_data_test):
    ...
    for tel_id in tel_ids:
        event_coord_true = SkyCoord(
            shower_data_test.loc[
                (slice(None), slice(None), tel_id), 'true_az'].values * u.rad,
            shower_data_test.loc[
                (slice(None), slice(None), tel_id), 'true_alt'].values * u.rad,
            frame=AltAz()
        )

        event_coord_reco = SkyCoord(
            shower_data_test.loc[
                (slice(None), slice(None), tel_id), 'az_reco'].values * u.rad,
            shower_data_test.loc[
                (slice(None), slice(None), tel_id), 'alt_reco'].values * u.rad,
            frame=AltAz()
        )
...
def train_direction_rf_stereo(config_file):
    ...
    for ei in range(len(energy_edges) - 1):
        ...
        for pi in [0]+tel_ids:
            ...
            try:
                selection = query.loc[
                    (slice(None), slice(None), tel_id), f'sep_{pi}'].dropna()
    ...
    for index, tel_id in enumerate([0]+tel_ids):
        ...
        plt.hist(separation_df[f'sep_{tel_id}'], bins=400, range=(0, 5),
                 cumulative=True, density=True, alpha=0.1, color='C0')
        plt.hist(separation_df[f'sep_{tel_id}'], bins=400, range=(0, 5),
                 cumulative=True, density=True, histtype='step', color='C0')
        plt.grid(linestyle=':')
