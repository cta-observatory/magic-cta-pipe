...
def main():
    ...
    )  # READ LIST OF DATES AND RUNS: format table in a way that each line looks like "2020_11_19,5093174"
    ...
    if (args.analysis_type == "onlyMC") or (args.analysis_type == "doEverything"):
        ...
        lists_and_bash_generator(
            "protons", target_dir, MC_protons, SimTel_version, focal_length, env_name
        )  # protons
        lists_and_bash_generator(
            "gammadiffuse",
            target_dir,
    ...
    if (args.analysis_type == "onlyMAGIC") or (args.analysis_type == "doEverything"):
        lists_and_bash_gen_MAGIC(
            target_dir, telescope_ids, MAGIC_runs, env_name
        )  # MAGIC real data
        if (telescope_ids[-2] > 0) or (telescope_ids[-1] > 0):
