...
# TODO 
# add dl0->dl1 for mcs (later)
# option for config.yaml with different name
# divide mcs into test and train
# class for stereo recos?
# try except e.g. in case one of the subruns fails (check if they work?)
...
def train_rfs(output_path, rf):
	try:
		...
		logger.info(f"/n{rf}-RF could not be trained.")
...
def main():
	...
	start = config["Start-at"]
	stop = config["Stop-after"]

	steps = ["calib_to_dl1", "dl1_to_dl1_stereo", "train_RFs", "dl1_stereo_to_dl2", "create_IRF", "dl2_to_dl3", "dl3_index"]
	while steps[0] != start:
		steps.pop(0)
	while steps[-1] != stop:
		steps.pop()
	...
	if name in steps:
		...
		input_file_masks = [input_path+date+"/M*/*_M*_"+runnum+".001_Y_CrabNebula-W0.*.root" for runnum in runnumbers for date in dates]
		...
		calib_to_dl1_time = start_time
	name = "dl1_to_dl1_stereo"
	if name in steps:
