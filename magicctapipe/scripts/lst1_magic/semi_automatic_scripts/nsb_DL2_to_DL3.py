...
#__version__='0.3.2.dev308+gd4cbcb9.d20240203'
...
def DL2_to_DL3(scripts_dir, target_dir, nsb, source):
    ...
    for p in ST_list:
        ...
        IRF_dir = (
            f"/fefs/aswg/workspace/elisa.visentin/MAGIC_LST_analysis/OP313/IRF/"
            + str(p)+"/NSB"+ str(nsb) 
        )  # IRF direcctory
        ...
        for night in nights:
            ...
            for DL2_file in listOfDL2files:
                ...
                f.write(
                    f"conda run -n OP313_env python {scripts_dir}/lst1_magic_dl2_to_dl3.py --input-file-dl2 {DL2_file} --input-dir-irf {IRF_dir} --output-dir {output} --config-file {target_dir}/config_DL3.yaml >$LOG 2>&1\n\n"
                )
