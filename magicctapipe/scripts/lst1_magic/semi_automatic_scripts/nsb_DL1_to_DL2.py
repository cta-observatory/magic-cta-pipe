...
def DL1_to_2(scripts_dir, target_dir, nsb, source, config, env_name):
    ...
    for p in ST_list:
        ...
        RFs_dir = (
            f"/fefs/aswg/workspace/elisa.visentin/MAGIC_LST_analysis/OP313/RF_34/"
            + str(p)
            + "/NSB"
            + str(nsb)
        ...
        for night in listOfDL1nights:
            ...
            with open(f'{source}_DL1_to_DL2_{nsb}_{night.split("/")[-1]}.sh', "w") as f:
                ...
                f.write(
                    f"conda run -n {env_name} python {scripts_dir}/lst1_magic_dl1_stereo_to_dl2.py --input-file-dl1 $SAMPLE --input-dir-rfs {RFs_dir} --output-dir {output} >$LOG 2>&1\n\n"
                )
