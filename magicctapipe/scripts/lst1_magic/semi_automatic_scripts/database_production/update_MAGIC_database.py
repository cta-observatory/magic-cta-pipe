...
def update_tables(database, DF, tel_id):    
    ...
    merged_df = pd.merge(database, DF[columns_to_compare], on=columns_to_compare, how='left', indicator=True)
    non_matching_rows = merged_df[merged_df["_merge"] == "left_only"].drop(columns=["_merge"])
    ...
    if non_matching_rows.empty:
        ...
        for index, row in non_matching_rows_reset.iterrows():
                ...
                if os.path.exists(path):
                    ...
                    if count_with_run_id != 0:
                        new_rows.append(
                            {"DATE": date, "source": source, "Run ID": run_id, "number of subruns": count_with_run_id}
                        )
        ...
        for column in combined_df.columns[4:]:
            ...
            combined_df[column] = combined_df[column].fillna(default_value).astype(type(not_null_data.iloc[0]))
...
def main():
    ...
    if file_exists:
        ...
        new_h5_file_path = "/fefs/aswg/workspace/joanna.wojtowicz/output/update_database.h5"
        
        try:
            updated_df_1.to_hdf(new_h5_file_path, key='MAGIC1/runs_M1', mode='w', format='table')
            updated_df_2.to_hdf(new_h5_file_path, key='MAGIC2/runs_M2', mode='a', format='table')
        ...
        try:
            database_M1.to_hdf(new_database_file_path, key='MAGIC1/runs_M1', mode='w', format='table')
            database_M2.to_hdf(new_database_file_path, key='MAGIC2/runs_M2', mode='a', format='table')
