...
def image_comparison(config_file = "config.yaml", mode = "use_ids_config"): 
	...
	for id_event in ids_to_compare:
		...
		with uproot.open(data_path) as input_data:
			event_ids = input_data['Events']['MRawEvtHeader.fStereoEvtNumber'].array(library="np")
			images = input_data['Events']['MCerPhotEvt.fPixels.fPhot'].array(library="np")
...
image_comparison(config_file = "image_comparison_config.yaml", mode = "use_ids_config")
