...
def compare_hillas_parameters(config_file ="config.yaml", hillas_key="dl1/hillas_params", subrun=5086952, data_or_mc="Data",
    test_or_train="Test", plot_image=False):
    """
    ...
    subrun: subrun number (only used for labeling the output, one could also add the run number without any problems)
    ...
    # read mcp data
    #--------------
    mcp_file = config["magic-cta-pipe-input"][data_or_mc][test_or_train]
    ...
    with uproot.open(config["MARS-input"]["MARS-path"]) as file_mars:
        for cut_par in cut_params:
            ...
            df_mars["mars_"+cut_par] = file_mars["Events"][mars_name].array(library ="np")#.to_numpy()
        for par in config["Hillas-params"]["Params"]:
            mars_name = mars_params[par]
            df_mars["mars_"+par] = file_mars["Events"][mars_name].array(library ="np")#.to_numpy()
    ...
    for par in config["Hillas-params"]["Params"]:
        if par[-1] == "1":
            ...
            df_hillas["relative_error"]=(df_hillas[par+"_mars"]-df_hillas[par+"_mcp"])/df_hillas[par+"_mcp"]    #may be infinity!!
        ...
        # plot image (optional)
        #----------------------
        #note: we dont use scaling factors here, since they are already taken into account in df_hillas -->see L67
        if plot_image == True:
            if scaling[par] == "log":
                plt.hist2d(df_hillas[f"{par}"+"_mcp"], df_hillas[f"{par}"+"_mars"], bins=np.logspace(np.log10(10), np.log10(10e5), 300),
                norm=colors.LogNorm(), cmap=plt.cm.jet, cmin=1) #you could also add a range parameter
                plt.colorbar()
                plt.loglog()
                plt.xlabel("magic-cta-pipe")
                plt.ylabel("MARS")
                plt.title(f"20191127_05086952_S - {labels_and_units[par]}")
                ...
                plt.hist2d(df_hillas[f"{par}"+"_mcp"], df_hillas[f"{par}"+"_mars"], bins=300, norm=colors.LogNorm(), cmap=plt.cm.jet, cmin=1)
                plt.colorbar()
                plt.xlabel("magic-cta-pipe")
                plt.ylabel("MARS")
                plt.title(f"20191127_05086952_S - {labels_and_units[par]}")
