...
from math import *
from params_for_hillas_comparison import (
    mars_params,
    mcp_params,
    scale_factors,
    threshold,
...
def compare_hillas_parameters(
    config_file="config.yaml",
    hillas_key="dl1/hillas_params",
    subrun=5086952,
    data_or_mc="Data",
    test_or_train="Test",
    plot_image=False,
    ...
    with uproot.open(config["MARS-input"]["MARS-path"]) as file_mars:
        ...
        for par in config["Hillas-params"]["Params"]:
            ...
            df_mars["mars_" + par] = file_mars["Events"][mars_name].array(
                library="np"
            )  # .to_numpy()
    ...
    # --------------------------
    df_merge_m1 = pd.merge(
        df_mars_cut, df_mcp_m1, on=["event_id"], how="inner"
    )
    df_merge_m2 = pd.merge(
        df_mars_cut, df_mcp_m2, on=["event_id"], how="inner"
    )
    comparison = []
    for par in config["Hillas-params"]["Params"]:
        if par[-1] == "1":
            ...
            df_hillas["relative_error"] = (
                df_hillas[par + "_mars"] - df_hillas[par + "_mcp"]
            ) / df_hillas[
                par + "_mcp"
            ]  # may be infinity!!
            error = df_hillas.loc[
                (df_hillas["relative_error"] > threshold[par])
            ].to_numpy()
        elif par[-1] == "2":
            df_hillas = pd.DataFrame()
            ...
            df_hillas["relative_error"] = (
                df_hillas[f"{par}" + "_mars"] - df_hillas[f"{par}" + "_mcp"]
            ) / df_hillas[f"{par}" + "_mcp"]
            error = df_hillas.loc[
                (df_hillas["relative_error"] > threshold[par])
            ].to_numpy()

        if (
            error.size <= len(df_hillas) * 0.01
            ...
            df_hillas.to_hdf(
                "{}/{}_hillas_{}.h5".format(
                    config["Output_paths"]["file_output_directory"],
                    subrun,
                    par,
                ),
                "/hillas_params",
        ...
        if plot_image == True:
            if scaling[par] == "log":
                plt.hist2d(
                    df_hillas[f"{par}" + "_mcp"],
                    df_hillas[f"{par}" + "_mars"],
                    bins=np.logspace(np.log10(10), np.log10(10e5), 300),
                    norm=colors.LogNorm(),
                    cmap=plt.cm.jet,
                    cmin=1,
                )  # you could also add a range parameter
                plt.colorbar()
                plt.loglog()
                ...
                plt.savefig(
                    "{}/{}_hillas_comparison_{}.png".format(
                        config["Output_paths"]["image_output_directory"],
                        subrun,
                        par,
                    ),
                    facecolor="w",
