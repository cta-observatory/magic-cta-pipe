...
def read_dl2_file(path):
    ...
    view_cone = run_header_tree["MMcCorsikaRunHeader.fViewconeAngles[2]"].array(
        library="np"
    )
    ...
    n_showers = np.sum(
        run_header_tree["MMcRunHeader_1.fNumSimulatedShowers"].array(library="np")
    )
...
def find_hadronness_cuts(
    ...
    had_vs_e, had_vs_e_xe, had_vs_e_ye = np.histogram2d(
        np.log10(reco_selected_h["reco_energy"]),
        reco_selected_h["hadroness"],
        bins=[nbinsE + 1, nbinsH + 1],
        range=[[np.log10(mineest) - e_step, np.log10(maxeest)], [-0.01 - h_step, 1.01]],
    )
    ...
    for ebin in range(1, nbinsE + 1):
        ...
        if np.sum(project) < 1:
            print(
                f"Energy bin {ebin} ({10**had_vs_e_xe[ebin]:.3f} < E < {10**had_vs_e_xe[ebin+1]:.3f} GeV): {hadcuts[ebin-1]:.5f}"
            )
            continue
...
def find_theta2_cuts(
    ...
    reco_selected_th = reco_selected_th[
        reco_selected_th["hadroness"] <= reco_selected_th["hadcut"]
    ]
    print(f"Number of events after events selection: {len(reco_selected_th.index)}")

    e_step = (np.log10(maxeest) - np.log10(mineest)) / nbinsE
    th_step = (0.4) / nbinsTH
    th_vs_e, th_vs_e_xe, th_vs_e_ye = np.histogram2d(
        np.log10(reco_selected_th["reco_energy"]),
        reco_selected_th["theta2"],
        bins=[nbinsE + 1, nbinsTH + 1],
        range=[[np.log10(mineest) - e_step, np.log10(maxeest)], [0 - th_step, 0.4]],
    )
    ...
    for ebin in range(1, nbinsE + 1):
        ...
        if np.sum(project) < 1:
            print(
                f"Energy bin {ebin} ({10**th_vs_e_xe[ebin]:.3f} < E < {10**th_vs_e_xe[ebin+1]:.3f} GeV): {thetacuts[ebin-1]:.3f}"
            )
            continue
...
p = dict()
p["events"], p["simulation_info"] = read_dl2_files(
    [
        "/storage/gpfs_data/ctalocal/aberti/Analysis_MAGIC/OFFS/Coach_ST0312/05to70/Melibea_MC/GA_za05to35_8_Q_w0_2.root"
    ]
)
gammas = p["events"]
...
hadcuts, had_vs_e = find_hadronness_cuts(
    reco, minsize, minzen, maxzen, mineest, maxeest, nbinsE, nbinsH
)
theta2cuts = find_theta2_cuts(
    reco, minsize, minzen, maxzen, mineest, maxeest, nbinsE, nbinsTH, hadcuts, had_vs_e
)
