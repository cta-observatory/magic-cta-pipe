...
def parse_args(args):
    ...
    parser.add_argument(
        "--calibrated", action="store_true", help="Save also calibrated images."
    )
    parser.add_argument(
        "-in",
        "--input_mask",
    ...
    parser.add_argument("--max_events", nargs="?", help="Maximum number of events.")
...
class ImageContainerCalibrated(Container):
    ...
    image_charge_calibrated = Field(
        None,
        "Numpy array of pixel charges before cleaning, after calibration."
        "Shape: (n_pixel)",
        dtype=">f8",
        ndim=1,
    )
    ...
    image_time_calibrated = Field(
        None,
        "Numpy array of pixel times before cleaning, after calibration."
        "Shape: (n_pixel)",
        dtype=">f8",
        ndim=1,
    )
...
def build_image_container_calibrated(
    run_number,
    event_id,
    tel,
    image_charge_calibrated,
    image_charge_cleaned,
    image_time_calibrated,
    image_time_cleaned,
):
    """
...
def build_image_container_cleaned(
    run_number, event_id, tel, image_charge_cleaned, image_time_cleaned
):
    """
        ...
        with HDF5TableWriter(
            ...
            with uproot.open(mars_file) as sstar:
                ...
                for batch in events.iterate(
                    step_size="10 MB", expressions=branches, library="np"
                ):
