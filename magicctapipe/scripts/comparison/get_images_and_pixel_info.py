...
arg_parser.add_argument(
    "--use_ids_config", help="Compare images for certain events", action="store_true"
)
...
if "input_files" not in config:
    print(
        'Error: the configuration file is missing the "input_files" section. Exiting.'
    )
    exit()

if "output_files" not in config:
    print(
        'Error: the configuration file is missing the "output_files" section. Exiting.'
    )
    exit()
...
if parsed_args.useall:
    ...
    if "Events" not in mcp_file.keys():
        ...
        if (
            ...
            not in mcp_file["Events"].keys()
        ):
            print(
                f"Missing key: MRawEvtHeader_{tel_id}.fStereoEvtNumber. Unacceptable file format. Exiting."
            )
            exit()
        else:
            df_mcp_data = pd.DataFrame()
            df_mcp_data["event_id"] = (
                mcp_file["Events"]["MRawEvtHeader./MRawEvtHeader.fStereoEvtNumber"]
                .array()
                .to_numpy()
            )
    ...
    for id_event in ids:
        ...
        for event in source._mono_event_generator(telescope=f"{telescope_id}"):
            ...
            for tel_id in [telescope_id]:
                ...
                bad_pixel_indices = [
                    i for i, x in enumerate(badrmspixel_mask[telescope_id - 1]) if x
                ]
                # print("badpixel_indices:", bad_pixel_indices)
                dead_pixel_indices = [
                    i for i, x in enumerate(deadpixel_mask[telescope_id - 1]) if x
                ]
        ...
        disp.highlight_pixels(
            np.array(event_image_mars[:1039]) != 0, color="white", alpha=1, linewidth=3
        )
        # disp.set_limits_minmax(vmin, vmax)
        # disp.add_colorbar(label='pixel charge')
        ...
        disp.highlight_pixels(
            np.array(event_image_cleaned) != 0, color="white", alpha=1, linewidth=3
        )
        # disp.set_limits_minmax(vmin, vmax)
        # disp.add_colorbar(label='pixel charge')
