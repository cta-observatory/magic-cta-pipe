...
def main():
    ...
    if args.base:
        ...
        event_images_stardump = np.genfromtxt(
            "%s/event_images.txt" % base_dir, dtype=float, max_rows=max_events
        )
        event_times_stardump = np.genfromtxt(
            "%s/event_times.txt" % base_dir, dtype=float, max_rows=max_events
        )
        event_unsuitables_stardump = np.genfromtxt(
            "%s/event_unsuitable.txt" % base_dir, dtype=bool, max_rows=max_events
        )
        event_unmappeds_stardump = np.genfromtxt(
            "%s/event_unmapped.txt" % base_dir, dtype=bool, max_rows=max_events
        )

        event_cleanstep1 = file_reader(
            "%s/event_cleanstep1.txt" % base_dir, max_row=max_events
        )
        event_cleanstep2 = file_reader(
            "%s/event_cleanstep2.txt" % base_dir, max_row=max_events
        )
        event_cleanstep3 = file_reader(
            "%s/event_cleanstep3.txt" % base_dir, max_row=max_events
        )
    ...
    if args.mc:
        ...
        # DATA FILES
        mars_file_mask = (
            "/home/iwsatlas1/damgreen/CTA/MAGIC_Cleaning/datafile/Calib/*.root"
        )
        mars_file_list = glob.glob(
            mars_file_mask
        )  # Here makes array which contains files matching the input condition (GA_M1*root).
    ...
    for ix, file_name in enumerate(file_list):
        ...
        for i, event in enumerate(event_generator):
            ...
            image_diff = (
                100 * np.abs(event_image_stardump - event_image) / event_image_stardump
            )
            image_diff_selection = image_diff > tolerance

            time_diff = (
                100
                * np.abs(event_time_stardump - event_pulse_time)
                / event_time_stardump
            )
            time_diff_selection = time_diff > tolerance

            ctapipe_unsuitable_mask = copy.copy(unsuitable_mask)
            star_unsuitable_mask = copy.copy(
                event_unsuitables_stardump[i + offset, :1039]
            )
            ...
            diff_unsuitable = np.setdiff1d(
                np.union1d(ctapipe_unsuitable_pixels, star_unsuitable_pixels),
                np.intersect1d(ctapipe_unsuitable_pixels, star_unsuitable_pixels),
            )
            diff_unmapped = np.setdiff1d(
                np.union1d(ctapipe_unmapped_pixels, star_unmapped_pixels),
                np.intersect1d(ctapipe_unmapped_pixels, star_unmapped_pixels),
            )
            ...
            diff_step1 = np.setdiff1d(
                np.union1d(ctapipe_step1, star_step1),
                np.intersect1d(ctapipe_step1, star_step1),
            )
            diff_step2 = np.setdiff1d(
                np.union1d(ctapipe_step2, star_step2),
                np.intersect1d(ctapipe_step2, star_step2),
            )
            diff_step3 = np.setdiff1d(
                np.union1d(ctapipe_step3, star_step3),
                np.intersect1d(ctapipe_step3, star_step3),
            )

            if (
                size_ratio > tolerance
                or length_ratio > tolerance
                or width_ratio > tolerance
            ):
                ...
                print(
                    bcolors.OKGREEN
                    + "Count : %i, Stereo Event Number : %s" % (i, event.dl0.event_id)
                    + bcolors.ENDC
                )
                ...
                if np.sum(image_diff_selection) > 0:
                    ...
                    print(
                        "image ctapipe : ", event_image[image_diff_selection].tolist()
                    )
                    print(
                        "image dump : ",
                        event_image_stardump[image_diff_selection].tolist(),
                    )
                    print(
                        "image diff (%) : ", image_diff[image_diff_selection].tolist()
                    )
                ...
                if np.sum(time_diff_selection) > 0:
                    ...
                    print(
                        "time ctapipe : ",
                        event_pulse_time[time_diff_selection].tolist(),
                    )
                    print(
                        "time dump : ",
                        event_time_stardump[time_diff_selection].tolist(),
                    )
    ...
    plt.text(
        0.15,
        0.9,
        "mean : %.4f" % np.nanmean(length_ratio),
        horizontalalignment="left",
        verticalalignment="center",
    ...
    plt.text(
        0.15,
        0.9,
        "mean : %.4f" % np.nanmean(width_ratio),
        horizontalalignment="left",
        verticalalignment="center",
