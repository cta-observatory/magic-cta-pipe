...
def linking_lst(target_dir, LST_runs, nsb, date):
    ...
    if len(LST_runs)==2:   ####TODO: BETTER SOLUTION
        LST=[]
        LST.append(LST_runs)
        LST.append([0,0])
        LST_runs=LST
...
def bash_coincident(target_dir, nsb):
    ...
    for nightMAGIC,nightLST in zip(listOfNightsMAGIC,listOfNightsLST):
        if not os.path.exists(nightLST+'/list_LST.txt'):
          continue
        ...
        f.write("#SBATCH -J "+process_name+"_coincidence"+str(nsb)+"\n")
...
def main():
    ...
    for nsblvl in nsb:
      try:
        ...
        print("Process name: "+target_dir.split("/")[-2:][1]+"_coincidence"+str(nsblvl))
        print("To check the jobs submitted to the cluster, type: squeue -n "+target_dir.split("/")[-2:][1]+"_coincidence"+str(nsblvl))
