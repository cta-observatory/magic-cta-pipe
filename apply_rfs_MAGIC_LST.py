...
def apply_rfs_stereo(config_file):
    ...
    for data_type in data_types:
        ...
        l_ = ["obs_id", "event_id"]
        shower_data["multiplicity"] = shower_data["intensity"].groupby(level=l_).count()
        ...
        mc_ = pd.read_hdf(
            cfg["data_files"][data_type][sample]["hillas_h5"], key="dl1/mc_header"
        )
        mc_.to_hdf(cfg["data_files"][data_type][sample]["reco_h5"], key="dl2/mc_header")
